...
class WeeklyReportOrchestrator:
    ...
    def _prepare_context(self) -> Dict[str, Any]:
        ...
        self._ctx = dict(
            ...
            week2_display = f"{(w2e-timedelta(days=7)).strftime('%d/%m/%Y')} → {w2e.strftime('%d/%m/%Y')}",
            week3_display = f"{(w3e-timedelta(days=7)).strftime('%d/%m/%Y')} → {w3e.strftime('%d/%m/%Y')}",
            week4_display = f"{(w4e-timedelta(days=7)).strftime('%d/%m/%Y')} → {w4e.strftime('%d/%m/%Y')}",
        )
        return self._ctx
