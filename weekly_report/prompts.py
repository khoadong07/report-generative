...
NHIỆM VỤ:
Với mỗi topic trong danh sách trên, viết 1 câu tóm tắt ngắn gọn (15–20 từ) mô tả sự vụ hoặc nội dung chính của chủ đề đó dựa trên các mẫu nội dung được cung cấp.

YÊU CẦU BẮT BUỘC:
- Mỗi topic viết đúng 1 câu, 15–20 từ
- Văn phong trung lập, súc tích, mô tả sự vụ cụ thể
- KHÔNG trích dẫn link, KHÔNG ghi nguồn
- KHÔNG dùng gạch đầu dòng, KHÔNG tiêu đề
- Xuất ra theo định dạng: [Tên topic]: [Câu tóm tắt]

VÍ DỤ FORMAT:
Khuyến mãi hè: Chương trình giảm giá mùa hè thu hút lượng lớn thảo luận từ người dùng trẻ.
Sản phẩm mới: Ra mắt dòng sản phẩm mới nhận được phản hồi tích cực từ cộng đồng mạng.
"""
...
def get_slide15_topic_sentiment_insight_prompt(
    ...
    for item in bar_data:
        ...
        nsr = summary_table["NSR"].get(item["topic"], 0)
        sent_lines.append(
...
NHIỆM VỤ:
Viết 4–5 câu insight phân tích sắc thái đề cập theo từng topic, chỉ ra topic nào có NSR cao/thấp nhất, chủ đề tích cực/tiêu cực nổi bật và hàm ý cho thương hiệu.

YÊU CẦU BẮT BUỘC:
- 4–5 câu, văn xuôi, không gạch đầu dòng, không tiêu đề
