...
from generators.weekly.slide_generators_weekly import (
    ...
    WeeklyNegativePostsGenerator,
    WeeklySlide13Generator,
)
...
class WeeklyReportGenerator:
    ...
    def __init__(self, api_key: str, base_url: str, file_path: str = None, 
                 ...
                 show_interactions: bool = True, competitor_brands: list = None):
        """
        ...
        Args:
            ...
            competitor_brands: List of competitor brand names for slide 11 comparison
        """
        ...
        # Slide 11: main brand + competitors (deduplicated, main brand always included)
        if competitor_brands:
            all_brands = [brand_name] + [b for b in competitor_brands if b != brand_name]
            self.slide11_brands = all_brands
        else:
            self.slide11_brands = None  # None = lấy tất cả Topic trong data
        ...
        self.slide1_gen = WeeklySlide1Generator(self.llm_client, TOPIC_TYPES)   # Tổng quan KPIs
        self.slide2_gen = WeeklySlide2Generator(self.llm_client, TOPIC_TYPES)   # Trendline 7 ngày
        self.slide3_gen = WeeklySlide3Generator(self.llm_client, TOPIC_TYPES)   # Phân bố kênh
        self.slide4_gen = WeeklySlide4Generator(TOPIC_TYPES, top_n=10)          # Top nguồn (table)
        self.slide5_gen = WeeklySlide5Generator(TOPIC_TYPES, top_n=10)          # Top bài đăng (table)
        self.slide6_gen = WeeklySlide6Generator(self.llm_client, TOPIC_TYPES)   # Sentiment + chủ đề
        self.slide7_gen = WeeklySlide7Generator(self.llm_client, TOPIC_TYPES)   # Chủ đề tích cực (chart)
        self.slide8_gen = WeeklySlide9Generator(TOPIC_TYPES, COMMENT_TYPES, top_n=10)   # Top bài đăng tích cực (table)
        self.slide9_gen = WeeklySlide10Generator(self.llm_client, TOPIC_TYPES)  # Chủ đề tiêu cực (chart)
        self.slide10_gen = WeeklyNegativePostsGenerator(TOPIC_TYPES, COMMENT_TYPES, top_n=10)  # Top bài đăng tiêu cực (table)
        self.slide11_gen = WeeklySlide12Generator(self.llm_client, TOPIC_TYPES) # So sánh thương hiệu
        self.slide12_gen = WeeklySlide13Generator(TOPIC_TYPES)                  # Multi-brand trendline
    ...
    def generate_report(self) -> Dict[str, Any]:
        ...
        # Keep full unfiltered data for slide 12 (brand comparison needs all topics)
        df_all = df.copy()
        ...
        # Filter full data (all topics) by time for slide 12 brand comparison
        week1_all_df = df_all[
            (df_all["PublishedDate"] > week1_end_dt - timedelta(days=7)) &
            (df_all["PublishedDate"] <= week1_end_dt)
        ].copy()
        week2_all_df = df_all[
            (df_all["PublishedDate"] > week2_end_dt - timedelta(days=7)) &
            (df_all["PublishedDate"] <= week2_end_dt)
        ].copy()
        ...
        def generate_slide9():
            ...
            result = self.slide9_gen.generate(
                week1_df, self.brand_name, week1_display
            )
        ...
        def generate_slide11():
            print("      [Slide 11] 🏆 Analyzing brand comparison...")
            result = self.slide11_gen.generate(
                week1_all_df, week2_all_df, self.brand_name, week1_display,
                brands_filter=self.slide11_brands
            )
            print("      [Slide 11] ✅ Completed")
            return ('slide_11', result)

        def generate_slide12():
            print("      [Slide 12] 📈 Generating multi-brand trendline...")
            result = self.slide12_gen.generate(
                week1_all_df, self.brand_name, week1_display,
                week1_start_dt.strftime('%Y-%m-%d'),
                week1_end_dt.strftime('%Y-%m-%d'),
                brands_filter=self.slide11_brands
            )
            print("      [Slide 12] ✅ Completed")
            return ('slide_12', result)
        ...
        with ThreadPoolExecutor(max_workers=7) as executor:
            futures = {
                ...
                executor.submit(generate_slide11): 'slide_11'
            }
            ...
            for future in as_completed(futures):
                try:
                    ...
                    completed += 1
                    print(f"      ⏱️  Progress: {completed}/7 LLM slides completed")
        ...
        slide8_data = self.slide8_gen.generate(week1_df, self.brand_name, week1_display)
        print("      [Slide 8] ✅ Completed")
        
        print("      [Slide 10] 📊 Generating top negative posts...")
        slide10_data = self.slide10_gen.generate(week1_df, self.brand_name, week1_display)
        print("      [Slide 10] ✅ Completed")

        print("      [Slide 12] 📈 Generating multi-brand trendline...")
        slide12_data = self.slide12_gen.generate(
            week1_all_df, self.brand_name, week1_display,
            week1_start_dt.strftime('%Y-%m-%d'),
            week1_end_dt.strftime('%Y-%m-%d'),
            brands_filter=self.slide11_brands
        ...
        report = {
            ...
            "slide_1": slides_data['slide_1'],   # Tổng quan KPIs
            "slide_2": slides_data['slide_2'],   # Trendline 7 ngày
            "slide_3": slides_data['slide_3'],   # Phân bố kênh
            "slide_4": slide4_data,              # Top nguồn (table)
            "slide_5": slide5_data,              # Top bài đăng (table)
            "slide_6": slides_data['slide_6'],   # Sentiment + chủ đề
            "slide_7": slides_data['slide_7'],   # Chủ đề tích cực (chart)
            "slide_8": slide8_data,              # Top bài đăng tích cực (table)
            "slide_9": slides_data['slide_9'],   # Chủ đề tiêu cực (chart)
            "slide_10": slide10_data,            # Top bài đăng tiêu cực (table)
            "slide_11": slides_data['slide_11'], # So sánh thương hiệu
            "slide_12": slide12_data,            # Multi-brand trendline + peak annotations
        }
